...
class TestNATSConfig:
    def test_defaults(self):
        cfg = NATSConfig()
        assert cfg.url == ""
        assert cfg.lightfall_prefix == "als.7011"
    ...
    def test_custom_values(self):
        cfg = NATSConfig(url="nats://broker:4222", lightfall_prefix="als.1234")
        assert cfg.url == "nats://broker:4222"
        assert cfg.lightfall_prefix == "als.1234"
    ...
    def test_app_config_from_dict(self):
        cfg = AppConfig(**{"nats": {"url": "nats://localhost:4222", "lightfall_prefix": "test.prefix"}})
        assert cfg.nats.url == "nats://localhost:4222"
        assert cfg.nats.lightfall_prefix == "test.prefix"
